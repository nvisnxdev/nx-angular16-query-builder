...
export class QueryBuilderComponent implements OnInit, OnChanges, ControlValueAccessor, Validator {
  ...
  getOperatorTextLabel(operator: string): string {
    if (this.config.getOperatorTextLabel && typeof this.config.getOperatorTextLabel === 'function') {
      const isCapitalize = !!this.config.capitalizeTextLabel;
      let textLabel: string = this.config.getOperatorTextLabel(operator);
      if (isCapitalize) {
        textLabel = textLabel.split(' ').map((a: string) => a.charAt(0).toUpperCase() + a.slice(1)).join(' ');
      }
      return textLabel;
    }
    return operator;
  ...
  getOperatorsSortable(field: string) {
    try {
      // default result
      ...
      const mapFields = {};
      const operatorsCache = this.getOperators(field);
      for (let operator of operatorsCache) {
        if (this.getOperatorTextLabel(operator)) {
          mapFields[this.getOperatorTextLabel(operator)] = operator;
        }
      }

      // default order
      let sortedFields: Array<any> = operatorsCache;

      // sort by label asc / desc
      if (this.config.sortOptions === 'asc') {
        sortedFields = Object.keys(mapFields).sort((a: string, b: string) => a.localeCompare(b));
      } else if (this.config.sortOptions === 'desc') {
        sortedFields = Object.keys(mapFields).sort((a: string, b: string) => b.localeCompare(a));
      } else if (Array.isArray(this.config.sortOptions)) {
        const mappedLabels: Array<string> = sortedFields.map(operator => this.getOperatorTextLabel(operator));
        let updateSortedFields: Array<string> = this.config.sortOptions.filter(a => mappedLabels.indexOf(a) > -1);
        let excessFields = mappedLabels.filter((a: string) => updateSortedFields.indexOf(a) < 0);
        sortedFields = updateSortedFields.concat(excessFields);
      }

      // to operators
      const convertToOperators = sortedFields.map((a: string) => mapFields[a]);
      return convertToOperators;
    } catch (err) {
      alert(err.message);
    }
  }
